				...
				if (!(-d < deltaTime && deltaTime < c)) {
					...
					return (
						<HitObject
							...
							tintColor={tintColor}
							hitObjectNumber={hitObjectNumber}
							hitObjectOpacity={hitObjectOpacity}
							key={"hitObject" + index}
						/>
					);
				else if (x[0] < 4) {
	...
	useEffect(() => {
		...
		openDB.onsuccess = function (dbOpening) {
			...
			db.transaction("Files").objectStore("Files").get(setId).onsuccess =
				async function (fileOpening) {
					const file = fileOpening.target.result.files.find(
						(x) => x.id == id
					);
					await sleep(300)
					let beatMap = await decodeBeatMap(file.file, setId, online); //Decode the beatmap
					await sleep(200);
