...
function SongSelectionMenu() {
  ...
  async function getOszFileToUnzip(file, diffe = 0, mode, mode2 = false) {
    ...
    for (let i in osuFiles) {
      ...
      if (!preview.includes(beatmapInfo[i].audioFile)) {
        ...
        beatmapInfo[i].audioFile = preview.indexOf(beatmapInfo[i].audioFile);
      }
  ...
  function keyaction(e) {
    ...
    if (previewSearch.style.opacity == 1) {
      ...
      if (e.key == "x" || e.key == "z") {
        {
          let doc = document.querySelector("#test23 > div >  div:hover ");
          if (doc != null) {
            doc.click();
  ...
  useEffect(() => {
    ...
    request.onsuccess = function (event) {
        ...
        async function (event) {
          setTimeout(() => {
            ...
            minis = new MiniSearch({
              fields: ["title", "artist", "creator", "tags", "setId"],
              storeFields: ["title", "artist", "creator", "tags", "setId"],
              searchOptions: {
                fuzzy: 0.2,
                prefix: true,
              },
      ...
      db.transaction("TempMeta").objectStore("TempMeta").getAll().onsuccess =
        async function (event) {
          setTimeout(() => {
            ...
            if (result1.length > 0) {
              ...
              request.onsuccess = function (event) {
                const db = event.target.result;
                const transaction = db.transaction(
                  ["TempAssets", "TempFiles", "TempMeta", "TempPreview"],
                  "readwrite"
                );
                for (let i2 in result1) {
                  let element = result1[i2];
                  let objectStore = transaction.objectStore("TempAssets");
                  objectStore.delete(element.setId);
                  objectStore = transaction.objectStore("TempFiles");
                  objectStore.delete(element.setId);
                  objectStore = transaction.objectStore("TempMeta");
                  objectStore.delete(element.setId);
                  objectStore = transaction.objectStore("TempPreview");
                  objectStore.delete(element.setId);
                }
              };
            }
  ...
  let list = metaData.map((element, index) => {
    return (
      <div
        ...
        id={"element" + index}
        style={{
          marginRight: -Math.abs(scrollIndex - index) * 10 + "px",
          marginLeft:
            (scrollIndex == index
              ? "-30"
            ...
            <div
              ...
              style={{
                backgroundImage: settingsVal.showBanners
                  ? "url(data:image/png;base64," + element.backgroundImage + ")"
                  : "",
                ...
                <div
                  ...
                  onClick={(e) => {
                    ...
                    request.onsuccess = function (event) {
                      ...
                      objectStore.delete(element.setId);
                      let temp = metaData;
                      temp = temp.filter((x) => x.setId != element.setId);
                      let temp2 = metaFiles;
                      temp2 = temp2.filter((x) => x.setId != element.setId);
                      let box = document.getElementById("element" + index);
                ...
                {element.levels.map((x, index2) => (
                  <div
                    ...
                    onClick={(e) => {
                      ...
                      if (secondaryIndex == index2) {
                        ...
                        previewCircleSize.style.width = x.circleSize * 10 + "%";
                        previewApproachRate.style.width =
                          x.approachRate * 10 + "%";
                        previewHPDrain.style.width = x.hpDrainRate * 10 + "%";
                        previewAccuracy.style.width = x.difficulty * 10 + "%";
                        ...
                        previewSong.innerHTML = metaData[index].title;
                        previewArtist.innerHTML = metaData[index].artist;
                  ...
                  >
                    ...
                    <div className=" pointer-events-none rounded-lg w-full  h-2 bg-white bg-opacity-30 ">
                      <div
                        className={" pointer-events-none rounded-lg h-2"}
                        style={{
                  ...
                  onClick={(e) => {
                    ...
                    fetch(
                      ...
                      .then((blob) => {
                        ...
                        getOszFileToUnzip(file, 0, true, false);
                      });
                  }}
                  style={{
                    opacity: deleteMode ? "1" : "1",
                    pointerEvents: true ? "all" : "none",
                    aspectRatio: exisiting.includes(element.setId)
                      ? "1 / 1000"
                      : "",
                  }}
                >
                  <div className="h-1/2 aspect-square">{donwloadIcon}</div>
                </div>
                      ...
                      if (secondaryIndex == index2) {
                        ...
                        fetch(
                          ...
                          .then((blob) => {
                            ...
                            unzippingSet.style.height = "3.5vh";
                            unzippingSet.style.opacity = "1";
                            getOszFileToUnzip(file, 0, true, true);
                          });
                        return;
                      } else {
                        setSecondaryIndex(index2);
                        ...
                        previewHPDrain.style.width = x.hpDrainRate * 10 + "%";
                        previewAccuracy.style.width = x.difficulty * 10 + "%";
                        previewMapper.innerHTML = element.creator;
                        try {
                          previewSource.innerHTML = utf8.decode(element.source);
                        } catch (_) {}
                        previewVersion2.innerHTML = x.level;
                        try {
                          previewTags.innerHTML = utf8.decode(element.tags);
                        } catch (_) {}
                        previewSong.innerHTML = element.title;
                        previewArtist.innerHTML = element.artist;
                        previewVersion.innerHTML = x.level;
                      }
                    }}
                    style={{
                      backgroundColor:
                        secondaryIndex == index2 ? "#94a3b844" : "",
                    }}
                  >
                    <div className=" pointer-events-none leading-[20px]">
                      {x.level}
    ...
    <>
      ...
      <div id="screen1">
        <div
          ...
          onLoad={onLD()}
          style={{
            opacity: onlineMode && webSearchData.length < 1 ? "0.5" : "0",
          }}
        >
        ...
        <div
          key={searchKey}
          id="scrollMenu"
          className=" duration-300 fixed overflow-y-scroll z-0 select-none  scroll-smooth overflow-x-hidden right-0  bg -post  pl-4   w-[100vw] items-end justify-end  grid   h-[150%] pt-[calc(50vh-88px)]    "
          onScroll={(e) => {
            if (deleteMode || metaData.length < 1) return;

            scrollMenu.style.scrollSnapType = "none";
            let children = scrollMenu.children;
			let clost = Math.min(parseInt(scrollMenu.scrollTop/88),metaData.length-1)
            ...
            if (
              ...
              sctimer = setTimeout(async function () {
                ...
                scrollMenu.style.marginTop =
                  -children[clost].getBoundingClientRect().height / 2 +
                  50 +
                  "px";
                ...
                if (clost != globalIndex) {
                  ...
                  setPreviewImage(metaData[clost].setId, x.backgroundImage, 0);

                  previewCircleSize.style.width = x.circleSize * 10 + "%";
                  previewApproachRate.style.width = x.approachRate * 10 + "%";
                  previewHPDrain.style.width = x.hpDrainRate * 10 + "%";
                  previewAccuracy.style.width = x.difficulty * 10 + "%";
                  previewMapper.innerHTML = x.mapper;
                  previewSource.innerHTML = utf8.decode(x.source);
                  ...
                  previewArtist.innerHTML = metaData[clost].artist;
                  previewVersion.innerHTML = x.level;
        ...
        <div
          ...
          onScroll={(e) => {
            ...
            let clost =  Math.min(parseInt(scrollMenu2.scrollTop/88),webSearchData.length-1)
            setScrollIndex(clost);
            if (
              ...
              sctimer = setTimeout(async function () {
                scrollMenu2.style.scrollSnapType = "y mandatory";
                await new Promise((r) => setTimeout(r, 10));
                sctimer = null;
                let children = scrollMenu2.children;
            let clost =  Math.min(parseInt(scrollMenu2.scrollTop/88),webSearchData.length-1)
                ...
                children[clost].style.height =
                  96 + webSearchData[clost].levels.length * 68 + "px";
                ...
                if (clost != globalIndex) {
                  ...
                  previewHPDrain.style.width = x.hpDrainRate * 10 + "%";
                  previewAccuracy.style.width = x.difficulty * 10 + "%";
                  previewMapper.innerHTML = webSearchData[clost].creator;
                  //console.log(webSearchData[clost].source);
                  try {
                    previewSource.innerHTML = webSearchData[clost].source;
                  } catch (_) {}
                  previewVersion2.innerHTML = x.level;
                  try {
                    previewTags.innerHTML = webSearchData[clost].tags;
                  } catch (_) {}
                  previewSong.innerHTML = webSearchData[clost].title;
                  previewArtist.innerHTML = webSearchData[clost].artist;
                  previewVersion.innerHTML = x.level;
        ...
        >
          ...
          >
              ...
              onClick={() => {
                ...
                if (dm) {
                  for (let i = 0; i < children.length - 1; i++) {
                    ...
                    children[i].style.marginRight = 10 + "px";
                    children[i].style.marginLeft = 10 + "px";
                    children[i].style.height = 80 + "px";
                  }
                } else {
                  setGlobalIndex(-1);
            ...
            >
              <div
                ...
                style={{
                  backgroundColor: onlineMode ? "#939393" : "",
              ...
              <input
                ...
                onChange={(e) => {
                  ...
                  typetimer = setTimeout(() => {
                    ...
                    if (onlineMode && osuDirect) {
                      ...
                      fetch(
                        ...
                        .then((data) => {
                          ...
                          let maps = [];
                          let ex = [];
                          let yx = metaFiles.map((x) => x.setId);
                          for (let i in res) {
                            let x = res[i];
                            if (yx.includes(x.id)) ex.push(x.id);
                            ...
                            maps.push({
                              artist: x.artist,
                              backgroundImage: x.covers["cover@2x"],
                              creator: x.creator,
                              levels: lev,
                              setId: x.id,
                              source: x.source,
                    ...
                    let temp = [];
                    for (let i in res) {
                      let val = metaFiles.find((x) => x.setId == res[i].setId);
          ...
          {focus && ((metaFiles.length > 0 && !onlineMode) || onlineMode) ? (
            <Preview />
          ) : (
            <></>
          )}
        ...
        >
          ...
          <button
            ...
            style={{
              ...
              pointerEvents: metaFiles.length < 1 ? "auto" : "none",
            }}
            onClick={(e) => {
              fetch(
          ...
          >
            ...
            >
              <div className="h-[3.5vh] scale-[60%]">{loader2}</div> Looking for
              beatmaps
            </div>
            <div
              id="fetchingSong"
            ...
            >
              <div className=" h-[3.5vh] scale-[60%]">{loader2}</div> Fetching
              preview audio
            </div>
            <div
              id="fetchingSet"
              className=" flex items-center opacity-0 overflow-hidden duration-300  h-0"
            >
              <div className=" h-[3.5vh] scale-[60%]">{loader2}</div>{" "}
              Downloading Beatmap
            </div>
            <div
              id="unzippingSet"
              className=" flex items-center opacity-0 overflow-hidden duration-300  h-0"
            >
              <div className=" h-[3.5vh] scale-[60%]">{loader2}</div> Unzipping
              Beatmap Set
            </div>
            <div
              id="setSaved"
            ...
            >
              <div className=" h-[3.5vh] opacity-0 scale-[60%]">{loader2}</div>{" "}
              Saved Successfully
            </div>
            <div
              id="waitDC"
